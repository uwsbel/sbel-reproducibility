...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        action = self.force_tensor[idx,]
        acceleration = self.accel_tensor[idx, :]
...
def train_force_PNODE(test_case, numerical_methods, model, body_tensor,force_tensor,accel_tensor, training_size,
                           step_delay=2, num_epochs=450, dt=0.01, device='cuda', verbose=True):
    # Setup logging
    def log(message):
        if verbose:
            print(message)
    log("Starting training of the PNODE model for force options")
    log(f"body_tensor shape: {body_tensor.shape}")
    log(f"Model: {model}")
    criterion = nn.MSELoss()
    #optimizer = optim.RAdam(model.parameters(), lr=0.0005)
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, training_size):
            optimizer.zero_grad()
            state = body_tensor[i, :, :].clone().to(device).requires_grad_(True)
            action = force_tensor[i,].clone().to(device).requires_grad_(True)
            acceleration = accel_tensor[i, :].clone().to(device).requires_grad_(True)
            #reshape the state to (num_bodys*2) and concatenate the state and action to form the input
            state=state.view(-1)
            #print("state shape is "+str(state.shape),"action shape is "+str(action.shape))
            inputs = torch.cat([state, action], dim=0)
            pred_acceleration = model(inputs)
            loss = criterion(pred_acceleration, acceleration)
            epoch_loss += loss.item()
            loss.backward()
            optimizer.step()
        print("loss is "+str(loss.item()))
...
# batch training
def train_force_batch_PNODE(test_case, numerical_methods, model, body_tensor,force_tensor,accel_tensor, training_size,
                           step_delay=2, num_epochs=450, dt=0.01, device='cuda', verbose=True):
    # Setup logging
    def log(message):
        if verbose:
    ...
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    losses = []
    your_batch_size=512*8
    # Assuming body_tensor, force_tensor, accel_tensor are your datasets
    dataset = MyDataset(body_tensor, force_tensor, accel_tensor)
    dataloader = DataLoader(dataset, batch_size=your_batch_size, shuffle=True)
    for epoch in range(num_epochs):
        epoch_loss = 0
        for state, action, acceleration in dataloader:
            optimizer.zero_grad()
            # No need for .clone().to(device).requires_grad_(True) as DataLoader handles this
            inputs = torch.cat([state.to(device), action.to(device)], dim=1)  # Adjust dim if needed
            pred_acceleration = model(inputs)
            loss = criterion(pred_acceleration, acceleration.to(device))
            epoch_loss += loss.item()
            loss.backward()
            optimizer.step()
        ...
        log(f'Epoch [{epoch + 1}/{num_epochs}], Loss: {epoch_loss / training_size:.10f}')
    return model
def test_force_PNODE(numerical_methods, model, body, force, num_steps, dt, device='cuda'):
    print("start testing the PNODE model for force options")
    print("body shape is "+str(body.shape))
    print("model is ",model)
    print("numerical_methods is "+numerical_methods)
    body_tensor = torch.tensor(body, dtype=torch.float32, device=device)
    force_tensor = torch.tensor(force, dtype=torch.float32, device=device)
    testing_result = torch.zeros(num_steps, body.shape[0], 2).to(device)
    for i in range(num_steps):
        inputs = torch.cat([body_tensor[i, :, :].to(device), force_tensor[i, :].to(device)], dim=1)
        outputs = model(inputs)
        testing_result[i, :, :] = outputs
    return testing_result
def test_force_batch_PNODE_trajectory(model, body_init, force, num_steps, dt, device='cuda'):
    print("start testing the PNODE model for force options")
    print("body shape is "+str(body_init.shape))
    print("model is ",model)

    #we now need to integrate the system using the trained model to predict the trajectory
    #we use the body_init as the initial condition
    #we use the force as the force we want to apply to the system
    #we use the num_steps as the number of steps we want to predict
    #we use the dt as the time step we want to use for the prediction
    #we use the device as the device we want to use for the prediction
    #we use the model as the trained model
    #we use the numerical_methods as the numerical methods we want to use for the prediction
    #we use the body_tensor_pred to store the predicted trajectory
    #Start prediction iteratively:
    num_body=body_init.shape[0]
    body_tensor_pred=torch.zeros(num_steps,num_body,2).to(device)
    body_tensor_pred[0,:,:]=body_init
    #make the force tensor to be 0 tensor here
    force=torch.zeros(num_steps,1).to(device)
    #use the midpoint method to integrate the system
    for i in range(num_steps-1):
        body_tensor_pred[i+1,:,:]=midpoint_control_single_step(body_tensor_pred[i,:,:],force[i,:],neural_network_force_function_PNODE,dt,model=model)

    return body_tensor_pred

def test_force_batch_PNODE_trajectory2(model, body_init, force, num_steps, dt, device='cuda'):
    print("start testing the PNODE model for force options")
    print("body shape is "+str(body_init.shape))
    print("model is ",model)
    num_body = body_init.shape[0]
    ...
    body_tensor_pred=torch.zeros(num_steps,num_body,2).to(device)
    body_tensor_pred[0,:,:]=body_init
    print(body_tensor_pred[0,:,:])
    #make the force tensor to be 0 tensor here
    force=torch.zeros(num_steps,2).to(device)
    #use the midpoint method to integrate the system
    for i in range(num_steps-1):
        body_tensor_pred[i+1,:,:]=midpoint_control_single_step(body_tensor_pred[i,:,:],force[i,:],force_cp_ext2,dt,model=None)
        #body_tensor_pred[i+1,:,:]=midpoint_control_single_step(body_tensor_pred[i,:,:],force[i,:],force_scd2,dt,model=model)
