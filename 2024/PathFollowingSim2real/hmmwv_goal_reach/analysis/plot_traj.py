...
for idx in exp_index:
    for p_idx in policy_index:
        if p_idx == 1:
            ...
            plt.scatter(traj[-1, 1], traj[-1, 2], color='g',alpha=1.0, marker='o', s=300, label='Goal' if idx == exp_index[0] else "", zorder=5, edgecolors='none')
...
plt.savefig('./trajectories_l2.png',dpi=300)