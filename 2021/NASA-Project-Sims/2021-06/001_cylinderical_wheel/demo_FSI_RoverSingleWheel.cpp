...
const std::string out_dir = GetChronoOutputPath() + "FSI_Rover_Single_Wheel/";
std::string demo_dir;
...
void ShowUsage() {
    ...
    cout << "or to use default input parameters ./demo_FSI_RoverSingleWheel " << endl;
}
...
//------------------------------------------------------------------
void WritewheelVTK(std::shared_ptr<ChBody> Body, 
                      double radius, 
                      double length, 
                      int res, 
...
void SaveParaViewFiles(fsi::ChSystemFsi& myFsiSystem,
    ...
    if (save_output && std::abs(mTime - (this_frame)*frame_time) < 1e-9) {
        ...
        WritewheelVTK(wheel, wheel_radius, wheel_length, 100, SaveAsRigidObjVTK);
        RigidCounter++;
        cout << "\n--------------------------------\n" << endl;
...
void CreateSolidPhase(ChSystemSMC& mphysicalSystem,
    ...
    utils::AddBoxGeometry(chassis.get(), mysurfmaterial, ChVector<>(.1 * wheel_radius, .1 * wheel_radius, .1 * wheel_radius),
                          ChVector<>(0, 0, 0));
    ...
    utils::AddSphereGeometry(axle.get(), mysurfmaterial, 0.1 * wheel_radius, ChVector<>(0, 0, 0));
