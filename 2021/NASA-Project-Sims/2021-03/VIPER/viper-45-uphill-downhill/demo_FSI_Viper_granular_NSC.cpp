...
int main(int argc, char* argv[]) {
    ...
    size_t numSPH = 0;
    std::ofstream myBCE;
    myBCE.open("./BCE_trapezoid.txt", std::ios::trunc);    
    for (int i = 0; i < numPart; i++) {
        ...
        if(y_ini > 0.01*initSpace0){
            x_ini = x_ini - det_x;
        }
        else{
            x_ini = x_ini + det_x;
        }
        Real hL_top = 0.5; // half length of the top of the heap
        Real z_max;
        ...
        Real z_mid = z_max - 10*initSpace0;
        Real z_low = z_max - 15*initSpace0;
        if(z_ini < z_max){
            if(z_ini > z_mid){
...
void CreateSolidPhase(ChSystemNSC& mphysicalSystem,
    ...
    std::string my_BCE_path = "./BCE_trapezoid.txt";
    ...
    for (int i = 0; i < 4; i++) {
        ...
        auto revo_body_cr_lower = chrono_types::make_shared<ChLinkLockLock>(); 
        ...
        auto revo_body_cr_upper = chrono_types::make_shared<ChLinkLockLock>(); 
        ...
        auto revo_cr_sr_lower = chrono_types::make_shared<ChLinkLockLock>(); 
        ...
        auto revo_cr_sr_upper = chrono_types::make_shared<ChLinkLockLock>(); 
