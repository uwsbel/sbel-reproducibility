...
step_sizes = [1e-4, 2e-4, 4e-4, 1e-3, 2e-3, 4e-3, 8e-3, 1e-2, 2e-2, 4e-2, 8e-2, 1e-1]
...
def run_model(args):
    ...
    pos_exact, vel_exact, acc_exact, _, _ = model_fn(['--form', form, '--mode', 'kinematics', '--tol', '1e-12', '--step_size', str(1e-4)])
    ...
    for i, steps in enumerate(step_sizes):
        ...
        try:
            ...
            print('{}-{}, step: {}, tol: {} overflowed'.format(form, pretty_name, str(steps), str(tols[i])))
...
for model_fn in [run_four_link, run_slider_crank]: #[run_single_pendulum, run_four_link, run_slider_crank]:
    num_bodies = 1 if model_fn.__name__ == 'run_single_pendulum' else 3

    for form in ['reps', 'rp']:#['reps', 'rp', 'rA']:
