import sys
sys.path.append('/home/unjhawala/projectlets/model-repo/simple-vehicles/lang-c/interfaces') #change this before final submission
...
points_data = data2.iloc[:,1:3]
points_response = model_post_mean[:,1:3]
lv2 = np.sum(np.sqrt((points_data.iloc[:1000,1] - points_response[:1000,1])**2 + (points_data.iloc[:1000,0] - points_response[:1000,0])**2))/points_data.shape[0]


points_data = data3.iloc[:,1:3]
points_response = model_post_mean[:,1:3]
lv3 = np.sum(np.sqrt((points_data.iloc[:1000,1] - points_response[:1000,1])**2 + (points_data.iloc[:1000,0] - points_response[:1000,0])**2))/points_data.shape[0]


points_data = data4.iloc[:,1:3]
points_response = model_post_mean[:,1:3]
lv4 = np.sum(np.sqrt((points_data.iloc[:1000,1] - points_response[:1000,1])**2 + (points_data.iloc[:1000,0] - points_response[:1000,0])**2))/points_data.shape[0]
...
if(save):
    ...
    mpl.savefig(f'./images/art_lat_{test_type}.png', facecolor = 'w', dpi = 600)
mpl.show()
print(f"Test 1 = {lv1}")
print(f"Test 2 = {lv2}")
print(f"Test 3 = {lv3}")
