...
double rock_scale = 0.2;

double wheel_rad = 0.305;
double wheel_ang = 0.5 * CH_C_PI;
...
int main(int argc, char* argv[]) {
    ...
    ChVector<> pos1 = ChVector<>(-100, 0, 0.46);
    ChVector<> pos2 = ChVector<>(-1.5, 0, 0.46);
    ...
    for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
        ...
        auto Body_1 = rover_one->GetChassis()->GetBody();
        auto ground = mphysicalSystem.Get_bodylist()[0];
        double rover_vel = Body_1->GetPos_dt().x();
        double rover_slip = 1.0 - rover_vel / (wheel_ang * wheel_rad);
        double spring_extension = spring->GetLength() - spring->GetRestLength();
        double spring_force = spring->GetForce();
        double wheel_torque = rover_one->GetWheelTracTorque(ViperWheelID::V_LF)
                            + rover_one->GetWheelTracTorque(ViperWheelID::V_RF)
                            + rover_one->GetWheelTracTorque(ViperWheelID::V_LB)
                            + rover_one->GetWheelTracTorque(ViperWheelID::V_RB);
        if (mphysicalSystem.GetChTime() > 0.0){
            std::cout << mphysicalSystem.GetChTime() << ", "
                      << rover_vel << ", "
                      << wheel_ang << ", "
                      << rover_slip << ", "
                      << spring_extension << ", "
        ...
        double buff_time = 3.0;
        if (mphysicalSystem.GetChTime() > buff_time && add_spring){
        ...
        if (mphysicalSystem.GetChTime() > buff_time){
            dbp << mphysicalSystem.GetChTime() - buff_time
                << rover_vel
                << wheel_ang
                << rover_slip
                << spring_extension
    ...
    dbp.write_to_file(out_dir + "/dbp_sph_5.csv");
...
void CreateSolidPhase(ChSystemNSC& mphysicalSystem, 
    ...
    driver1 = chrono_types::make_shared<ViperSpeedDriver>(0.1, wheel_ang);
    ...
    rover_one->Initialize(ChFrame<>(ChVector<>(paramsH->bodyIniPosX, paramsH->bodyIniPosY, paramsH->bodyIniPosZ), 
        Q_from_Euler123(ChVector<double>(0, 0, 0))));
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if(i==0){
            CreateMeshMarkers(trimesh, (double)initSpace0, BCE_wheel);
        }
        myFsiSystem.AddFsiBody(wheel_body);
        if (i == 0 || i == 2) {
            myFsiSystem.AddBceFromPoints(paramsH, wheel_body, BCE_wheel, ChVector<>(0), Q_from_AngZ( CH_C_PI));
        } else {
            myFsiSystem.AddBceFromPoints(paramsH, wheel_body, BCE_wheel, ChVector<>(0), QUNIT);
        }
    }
    ...
    for (int j = 0; j < 1; j++) {
        ...
        rot_ang_x = 1 * 90.0 / 180.0 * CH_C_PI;
        rot_ang_y = j * 45.0 / 180.0 * CH_C_PI;
        rot_ang_z = i * 60.0 / 180.0 * CH_C_PI;
        // double det_x = 0.1 * pow(-1.0, i + j);
        // double det_y = 0.1 * pow(-1.0, i + j + 1);
        // ChVector<> rock_pos = ChVector<>(-2.0 + i * 1.0 + det_x, -2.0 + j * 1.0 + det_y, 0.6);
        ChVector<> rock_pos = ChVector<>(0.0, 0.0, 0.6);
