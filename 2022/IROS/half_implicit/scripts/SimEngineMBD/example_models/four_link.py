...
def setup_four_link(args=None):
    ...
    # radius/width/thickness of rod or rotor disk
    w = 0.1
    rho = 1500
    # See Haug p. 459 for properties
    # Link 1
    sys.bodies[0].m = 2
    sys.bodies[0].J = np.diag([4, 2, 2])
    sys.bodies[0].J_inv = np.diag([1/4, 1/2, 1/2])
    # rotor_radius = 2
    # rotor_volume = np.pi * rotor_radius**2 * w
    # rotor_mass = rotor_volume * rho
    # rotor_Jxx = 0.25*rotor_mass*rotor_radius**2
    # rotor_Jyy = rotor_Jxx
    # rotor_Jzz = 0.5*rotor_mass*rotor_radius**2

    # sys.bodies[0].m = rotor_mass
    # sys.bodies[0].J = np.diag([rotor_Jxx, rotor_Jyy, rotor_Jzz])
    # sys.bodies[0].J_inv = np.diag([1/rotor_Jxx, 1/rotor_Jyy, 1/rotor_Jzz])

    # Link 2
    sys.bodies[1].m = 1
    sys.bodies[1].J = np.diag([12.4, 0.1, 12.4])
    sys.bodies[1].J_inv = np.diag([1/12.4, 1/0.01, 1/12.4])
    # link2_len = 12.2
    # link2_volume = np.pi * w**2 * link2_len
    # link2_mass = rho * link2_volume
    # link2_Jxx = 1/12 * link2_mass * link2_len**2
    # link2_Jyy = 0.5 * link2_mass * w**2
    # link2_Jzz = link2_Jxx
    # sys.bodies[1].m = link2_mass
    # sys.bodies[1].J = np.diag([link2_Jxx, link2_Jyy, link2_Jzz])
    # sys.bodies[1].J_inv = np.diag([1/link2_Jxx, 1/link2_Jyy, 1/link2_Jzz])

    # Link 3
    sys.bodies[2].m = 1
    sys.bodies[2].J = np.diag([4.54, 0.01, 4.54])
    sys.bodies[2].J_inv = np.diag([1/4.54, 1/0.01, 1/4.54])
    # link3_len = 7.4
    # link3_volume = np.pi * w**2 * link3_len
    # link3_mass = rho * link3_volume
    # link3_Jxx = 1/12 * link3_mass * link3_len**2
    # link3_Jyy = 0.5 * link3_mass * w**2
    # link3_Jzz = link3_Jxx
    # sys.bodies[2].m = link3_mass
    # sys.bodies[2].J = np.diag([link3_Jxx, link3_Jyy, link3_Jzz])
    # sys.bodies[2].J_inv = np.diag([1/link3_Jxx, 1/link3_Jyy, 1/link3_Jzz])
