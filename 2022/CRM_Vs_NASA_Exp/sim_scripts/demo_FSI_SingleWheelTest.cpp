...
double wheel_AngVel = wheel_vel / (wheel_radius * (1.0 - wheel_slip));
double total_mass = 17.5;
std::string wheel_obj = "robot/viper/obj/viper_simplewheel.obj";

// Initial Position of wheel
ChVector<> wheel_IniPos(-bxDim / 2 + wheel_radius * 2.0 , 0.0, 0.0);
...
std::string out_dir = GetChronoOutputPath() + "Single_Wheel_Test_1108/VV_mode_GRC3_42deg";
// ChVector<> density_s(1500.0, 1650.0, 1800.0);
// std::string file_name[3]={{"37deg"},{"42deg"},{"47deg"}};
...
void CreateSolidPhase(ChSystemSMC& sysMBS, ChSystemFsi& sysFSI) {
    ...
    sysFSI.AddWheelBCE_Grouser(wheel, ChVector<>(0), ChQuaternion<>(1, 0, 0, 0), inner_radius, 
        wheel_wide + iniSpacing, grouser_height, grouser_wide, grouser_num, kernelLength, false);
    // sysFSI.AddCylinderBCE(wheel, ChVector<>(0), ChQuaternion<>(1, 0, 0, 0), wheel_radius, 
    //                     wheel_wide + iniSpacing, kernelLength, false);
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        wheel_slip = 0.2 * (std::stoi(argv[1]) % 5);
        // density = density_s[std::stoi(argv[1]) / 5];
        wheel_AngVel = wheel_vel / ( wheel_radius * (1.0 - wheel_slip) );
        out_dir = out_dir + "/slip0_" + std::to_string(2 * (std::stoi(argv[1]) % 5)) + "/";
    } else if (argc != 1) {
