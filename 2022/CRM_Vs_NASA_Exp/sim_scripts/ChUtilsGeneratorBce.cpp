...
// =============================================================================
void CreateBCE_On_Wheel_Grouser(thrust::host_vector<Real4>& posRadBCE,
                           Real wheel_rad,
                           Real wheel_w,
                           Real gro_h,
                           Real gro_w,
...
void CreateBCE_On_ChElementShellANCF(thrust::host_vector<Real4>& posRadBCE,
    ...
    double dz = shell->GetLayer(0).Get_thickness() / 2;

    double nX = dx / initSpace0 - std::floor(dx / initSpace0);
    double nY = dy / initSpace0 - std::floor(dy / initSpace0);
    double nZ = dz / initSpace0 - std::floor(dz / initSpace0);
    int nFX = (int)std::floor(dx / initSpace0);
    int nFY = (int)std::floor(dy / initSpace0);
    int nFZ = (int)std::floor(dz / initSpace0);
    if (nX > 0.5)
        nFX++;
    if (nY > 0.5)
        nFY++;
    ...
    if ( dz < nFZ*initSpace0 ) {
        kBound = mI2(-nFZ*2, nFZ*2);
        initSpaceZ = dz / nFZ;
        }
    else {
        kBound = mI2(-nFZ*2-1, nFZ*2+1);
        initSpaceZ = dz / ( 0.5 + nFZ);
        }
    // int2 kBound;
    ...
    for (int k = kBound.x; k <= kBound.y; k=k+2) {
        ...
        for (int j = jBound.x; j <= jBound.y; j=j+2) {
            for (int i = iBound.x; i <= iBound.y; i=i+2) {
                Real3 relMarkerPos = mR3(i * initSpaceX / 2.0 , j * initSpaceY / 2.0 , k * initSpaceZ / 2.0);
