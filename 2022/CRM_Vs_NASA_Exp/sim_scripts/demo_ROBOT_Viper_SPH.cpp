...
double mass_scale = 73.0 / 440.0; // 1.0 means using default mass 440kg
double wheel_AngVel = 0.8;

// output directories and settings
std::string out_dir = GetChronoOutputPath() + "FSI_Viper_test/440kg_Earth_angvel0_8_GRC3_42deg/angle_";
...
double slope_angle = 0.0 / 180.0 * CH_C_PI;
...
int main(int argc, char* argv[]) {
    ...
    double rover_mass = rover->GetRoverMass();
    std::cout << "  rover_mass: " << rover_mass << std::endl;

    auto wheel_body = rover->GetWheel(ViperWheelID::V_LF)->GetBody();
    ...
    while (time < total_time) {
        ...
        std::cout << "  pos: " << wheel_body->GetPos() << std::endl;
        std::cout << "  vel: " << wheel_body->GetPos_dt() << std::endl;
        if (output) {
            ofile << time << "  " << wheel_body-> GetPos() << "  " << wheel_body->GetPos_dt() 
                         << "  " << wheel_body-> GetWvel_loc() << std::endl;
...
void CreateSolidPhase(ChSystemNSC& sysMBS, ChSystemFsi& sysFSI) {
    ...
    for (int i = 0; i < 17; i++) {
        // std::shared_ptr<ChBodyAuxRef> viper_part;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        sysFSI.AddWheelBCE_Grouser(wheel_body, ChVector<>(0), ChQuaternion<>(1, 0, 0, 0), inner_radius, 
            wheel_wide + initSpace0, grouser_height, grouser_wide, grouser_num, kernelLength, false);
