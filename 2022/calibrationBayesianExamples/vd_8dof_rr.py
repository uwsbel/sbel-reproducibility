...
def loglike(theta,data):
    ...
    Cf=44016,Cr=54046,r0=0.47,ktf=326332,ktr=326332,krof=36245,kror=26439,brof=3863,bror=3863,hrcf=0.379,hrcr=0.327,Jw=11,
    Cxf = 10659,Cxr = 10280,rr=theta[0])
    ...
    return -np.sum(((n*np.log(2*np.pi * sigmas**2)/2) + np.sum((mod[[2],:] - data)**2/(2.*sigmas**2)))/np.linalg.norm(data,axis = 1))
...
def main():
    ...
    nburn = int(ndraws/2)
    with open('data/vd_chrono_rr_1.npy', 'rb') as f:
        data = np.load(f)
