...
bool output = true;
std::string out_dir = GetChronoOutputPath() + "SCM_Viper/528kg_Moon_angvel0_8_GRC1_37deg/angle_";

// Terrain type
int terrain_type = 1; // 0: GRC-3, 1: GRC-1

// wheel specifics
double wheel_rad = 0.25;
double wheel_slip = 0.0;
double wheel_vel = 0.2;
double wheel_ang = 0.8;

// Earth or Moon gravity
double gravity_G = -1.62;

// Full rover mass, mass_scale = 1.0 means using default mass 440kg
double mass_scale = 528.0 / 440.0;

// Rover initial location
ChVector<> init_loc(-3.5, 0, wheel_rad + 0.02);
...
int main(int argc, char* argv[]) {
    ...
    out_dir = out_dir + std::to_string(std::stoi(argv[1]) * 2) + "/";

    double slope_angle = (std::stod(argv[1]) * 2.0) / 180.0 * CH_C_PI;
    ...
    if (var_params) {
        ...
        // GRC-3 
        if (terrain_type == 0){
            terrain.SetSoilParameters(3.66e6,  // Bekker Kphi
                                     -1.55e5,  // Bekker Kc
                                      1.29,    // Bekker n exponent
                                      2044,    // Mohr cohesive limit (Pa)
                                      25,      // Mohr friction limit (degrees)
                                      0.0114,  // Janosi shear coefficient (m)
        ...
        // GRC-1
        if (terrain_type == 1){
            terrain.SetSoilParameters(2.99e6,  // Bekker Kphi
                                     -2.02e5,  // Bekker Kc
                                      1.27,    // Bekker n exponent
                                      1587,    // Mohr cohesive limit (Pa)
                                      21,      // Mohr friction limit (degrees)
                                      0.0184,  // Janosi shear coefficient (m)
                                      4e7,     // Elastic stiffness (Pa/m), before plastic yield, must be > Kphi
                                      3e4      // Damping (Pa s/m), proportional to negative vertical speed (optional)
            );
        }
    ...
    for (int i = 0; i < 17; i++) {
        auto viper_part = sys.Get_bodylist()[i];
    ...
    while (time < total_time) {
        ...
        wheel_vel = 0.25 * (Wheel_1->GetPos_dt().x() + Wheel_2->GetPos_dt().x() + 
                            Wheel_3->GetPos_dt().x() + Wheel_4->GetPos_dt().x());
        // wheel_ang = 0.25 * (Wheel_1->GetWvel_loc().z() + Wheel_2->GetWvel_loc().z() + 
        //                     Wheel_3->GetWvel_loc().z() + Wheel_4->GetWvel_loc().z());
        wheel_slip = 1.0 - wheel_vel / (wheel_ang * wheel_rad);
