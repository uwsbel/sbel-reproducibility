...
double wheel_width = 0.28;
double wheel_slip = 0.0;
double wheel_velocity = 0.25;
double wheel_AngVel = wheel_velocity / ( wheel_radius * (1.0 - wheel_slip) );
double total_mass = 108.22;
std::string wheel_obj = "robot/viper/obj/viper_wheel.obj";

// Output directories and settings
std::string out_dir = GetChronoOutputPath() + "FSI_Single_Wheel_Test/Real_VIPER_Wheel/new_dx10mm_h10mm_height15cm_24mmcyl/01-108kg-slip00";
...
void CreateSolidPhase(ChSystemSMC& sysMBS, ChSystemFsi& sysFSI) {
    ...
    sysFSI.AddCylinderBCE(wheel, ChVector<>(0), QUNIT, wheel_radius - 0.01, 
        wheel_width + iniSpacing, sysFSI.GetKernelLength(), false);
    ...
    auto actuator_fun = chrono_types::make_shared<ChFunction_Ramp>(0.0, wheel_velocity);
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        ...
        wheel_AngVel = wheel_velocity / ( wheel_radius * (1.0 - wheel_slip) );
    ...
    while (time < total_time) {
        ...
        double sinkage = wheel_radius + bzDim - w_pos.z();
        ...
        if (output) {
            myFile << time << "\t" << w_pos.x() << "\t" << w_pos.y() << "\t" << w_pos.z() << "\t" << w_vel.x() << "\t"
                   ...
                   << torque.y() << "\t" << torque.z() << "\t" << sinkage << "\t" << wheel_slip << "\n";

            myDBP << time << "\t" << wheel_slip << "\t" << sinkage << "\t" 
                  << force.x() << "\t" << force.y() << "\t" << force.z() << "\n";

            myTorque << time << "\t" << wheel_slip << "\t" << sinkage << "\t" 
