...
// Size of the plate
double p_length = 0.05;
double p_radius = 0.2;
double p_radius_in = 0.15;
double linVel = 0.0;
double angVel = 0.5 * CH_C_PI / 180.0;
double total_mass = 200.0;
// double load_pressure = 1.25 * 6895;
// double load_force = CH_C_PI * (p_radius * p_radius - p_radius_in * p_radius_in) * load_pressure;

// Output directories and settings
const std::string out_dir = GetChronoOutputPath() + "FSI_Bevameter/31_r20cm_r15cm_200kg_05deg/";

double cohesion = 1000.0;

// Cylinder Annulus is for rotation test, set to false when doing normal test
bool Annulus = true;
...
void CreateSolidPhase(ChSystemSMC& sysMBS, ChSystemFsi& sysFSI) {
    ...
    chrono::utils::AddCylinderGeometry(plate.get(), mysurfmaterial, p_radius, p_length,
                                       ChVector<>(0.0), QUNIT);
    ...
    if (Annulus) {
        sysFSI.AddCylinderAnnulusBCE(plate, ChVector<>(0.0), QUNIT, p_radius_in, p_radius, p_length,
                            sysFSI.GetKernelLength(), false);
    } else {
        sysFSI.AddCylinderBCE(plate, ChVector<>(0.0), QUNIT, p_radius, p_length,
                            sysFSI.GetKernelLength(), false);        
    }
