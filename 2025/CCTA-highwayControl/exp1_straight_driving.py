...
from casadi import *
import time as tm
from controller import error_state, vir_veh_controller, mpc_wpts_solver_sedan, mpc_wpts_solver_sedan2, FindMinDistInd, CPWarningSystem, mpc_wpts_solver_sedan_dob
...
min_pause = 10.0  # Minimum pause between trajectories
max_pause = 20.0  # Maximum pause between trajectories
...
while vis.Run() :
    ...
    # if time > 30:
        ...
        if scenario_choice < 0.0:  # 40% chance for merge
            # Merge scenario
        ...
        elif scenario_choice < 0.0:  # 40% chance for parallel
            # Parallel scenario
            ...
            if not is_merging and not is_merging_left:
                # Serial scenario
                # driver_vel = np.random.uniform(20, 30)
                # run_time = np.random.uniform(3.5, 5.5)
                driver_vel = 12.0
                run_time = np.random.uniform(7, 8)
                
                serial_trajectory = serial_scenario(reference_trajectory, sedan_x, sedan_y,
                                                    deviation_distance=40,
                                                    vel=driver_vel,
                                                    run_time=run_time,
                                                    freq=1/render_step_size)
...
####################################### Controller design ######################################
    ...
    if (step_number % control_steps == 0):
        if is_manual:
            ...
            if (step_number % mpc_control_step == 0):
                ...
                start_time = tm.time()
                ctrl, _ = mpc_wpts_solver_sedan2(SV_pos, veh_state, xref, uref,mpc_step_size, N_mpc,0) # the last argument is penalty
    ...
    if (step_number % render_steps == 0) :
        ...
        filename = 'figs/balance/img_' + str(render_frame) +'.jpg' 
