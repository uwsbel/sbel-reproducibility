...
try:
    ...
    from Model.integrator import (
        forward_euler_multiple_body,
        runge_kutta_four_multiple_body,
        midpoint_method_multiple_body,
    )
    try:
        import torchdiffeq
        TORCHDIFFEQ_AVAILABLE = True
        torchdiffeq_odeint = torchdiffeq.odeint
    except ImportError:
...
def parse_arguments():
    ...
    parser.add_argument('--integrator', type=str, default='rk4',
                        choices=['fe', 'rk4', 'midpoint', 'dopri5'],
                        help="Integrator for trajectory prediction: fe (forward Euler), rk4, midpoint, or dopri5.")
...
def generate_multi_friction_datasets(args):
    ...
    if use_cache:
        ...
        try:
            ...
            if np.allclose(cached_c_train, c_train_values) and np.allclose(cached_c_test, c_test_values):
                ...
                # Normalize to (s_data, t_data) format in case cache has 3-tuples (s_data, t_data, a_data)
                train_trajectories = [(t[0], t[1]) for t in train_trajectories]
                test_trajectories = [(t[0], t[1]) for t in test_trajectories]
    ...
    for c_val in c_train_values:
        ...
        s_data, t_data, _ = generate_slider_crank_dataset_with_friction(
            c_slide=c_val,
...
def _integrate_trajectory(model, s0_physical, t_data, c_val, c_max, norm_stats, args,
                         integrator_name, device):
    """
    Integrate trajectory using integrator from Model/integrator.py (fe, rk4, midpoint) or dopri5.

    Args:
        s0_physical: Initial state [1, 2] = [theta, omega] in physical space
        t_data: Time vector
        Returns: pred_states [num_steps, 2] in physical space
    """
    num_steps = len(t_data)
    dt = args.dt_sample
    c_normalized_val = c_val / c_max

    def force_function(state, model=None):
        """Compute acceleration from state [num_bodys, 2]. Returns [num_bodys]."""
        if model is None:
            return torch.zeros(state.shape[0], device=state.device, dtype=state.dtype)
        state_f32 = state.float().to(device)
        theta_norm = (state_f32[:, 0] - norm_stats['theta_mean']) / norm_stats['theta_std']
        omega_norm = (state_f32[:, 1] - norm_stats['omega_mean']) / norm_stats['omega_std']
        c_col = torch.full((state.shape[0], 1), c_normalized_val, dtype=torch.float32, device=device)
        model_input = torch.stack([theta_norm, omega_norm], dim=-1)
        model_input = torch.cat([model_input, c_col], dim=-1)
        with torch.no_grad():
            pred_accel_norm = model(model_input)
        pred_accel = pred_accel_norm.squeeze(-1) * norm_stats['accel_std'] + norm_stats['accel_mean']
        return pred_accel.to(state.dtype).to(state.device)

    integrator_name_actual = integrator_name
    if integrator_name == 'dopri5' and not TORCHDIFFEQ_AVAILABLE:
        logger.warning("dopri5 requested but torchdiffeq not available. Falling back to rk4.")
        integrator_name_actual = 'rk4'

    if integrator_name_actual == 'dopri5':
        def ode_func(t, state_flat):
            state = state_flat.view(1, 2)
            accel = force_function(state, model=model)
            vel = state[:, 1]
            return torch.cat([vel, accel], dim=0)

        s0_flat = s0_physical.squeeze(0).to(device)
        t_eval = t_data.to(device)
        pred_traj = torchdiffeq_odeint(ode_func, s0_flat, t_eval, method='dopri5', rtol=1e-7, atol=1e-9)
        return pred_traj.float()  # [num_steps, 2]

    # fe, rk4, midpoint from Model/integrator.py
    methods = {
        'fe': forward_euler_multiple_body,
        'rk4': runge_kutta_four_multiple_body,
        'midpoint': midpoint_method_multiple_body,
    }
    if integrator_name_actual not in methods:
        raise ValueError(f"Integrator '{integrator_name_actual}' not in fe, rk4, midpoint, dopri5")
    integrator_fn = methods[integrator_name_actual]

    bodys = s0_physical.to(device)
    if bodys.dtype != torch.float64 and integrator_name_actual == 'rk4':
        bodys = bodys.double()
    kwargs = dict(
        bodys=bodys,
    ...
    if integrator_name_actual == 'rk4':
        kwargs['device'] = device
    else:
        kwargs['current_device'] = device
    traj = integrator_fn(**kwargs)
    # traj shape: [num_steps, num_bodys, 2]
    pred_states = traj[:, 0, :].float()  # [num_steps, 2] for num_bodys=1
    return pred_states
...
def plot_results(model, train_trajectories, test_trajectories,
    ...
    Args:
        ...
        test_indices_to_show: List of indices for test trajectories to display.
                             If None, defaults to [0, -1] (first and last)
    """
    integrator_name = getattr(args, 'integrator', 'rk4')
    logger.info(f"=== Generating Main Results Visualization (integrator: {integrator_name}) ===")
    ...
    # Generate predictions for selected training trajectories using integrator
    for idx in train_indices_to_show:
        ...
        with torch.no_grad():
            ...
            s0_physical = s0_norm.clone()
            s0_physical[:, 0] = s0_norm[:, 0] * norm_stats['theta_std'] + norm_stats['theta_mean']
            s0_physical[:, 1] = s0_norm[:, 1] * norm_stats['omega_std'] + norm_stats['omega_mean']
            pred_states = _integrate_trajectory(
                model, s0_physical, t_data, c_val, args.c_max, norm_stats, args,
                args.integrator, device
            )
            selected_train_predictions.append(pred_states.cpu().numpy())

    # Generate predictions for selected test trajectories using integrator
    for idx in test_indices_to_show:
        ...
        with torch.no_grad():
            ...
            selected_test_predictions.append(pred_states.cpu().numpy())
...
def main():
    ...
    target_csv_path = generate_target_accelerations(
        ...
        num_bodies=1,
        args=args,
        target_dir=output_paths["results"]
    )
