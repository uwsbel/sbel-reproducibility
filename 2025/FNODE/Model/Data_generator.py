...
def generate_hnn_dataset(seed=0, samples=1, test_split=0.1, t_span=(0.0, 30.0),
                         timescale=100, mass=10.0, k_spring=50.0):
    """
    Generate dataset for Hamiltonian Neural Network (HNN) training.
    Uses Single Mass Spring in (q, p) canonical coordinates.
    Returns dict with keys: x, dx, test_x, test_dx (numpy arrays).
    """
    from Model.force_fun import hamiltonian_spring_dynamics

    np.random.seed(seed)
    dt = 1.0 / timescale
    t_start, t_end = t_span
    num_steps_total = int((t_end - t_start) / dt) * samples

    t_eval = np.linspace(t_start, t_end, int((t_end - t_start) / dt) + 1)
    # Initial state: q=x=1, p=m*v=0 (from x=1, v=0)
    x0 = np.array([1.0, 0.0])
    ...
    for _ in range(samples):
        sol = scipy.integrate.odeint(
            lambda t, y: hamiltonian_spring_dynamics(t, y, mass=mass, k_spring=k_spring),
            x0, t_eval, tfirst=True
        )
        qp = sol  # [num_steps, 2] as (q, p)
        dqp = np.array([hamiltonian_spring_dynamics(t, qp[i], mass=mass, k_spring=k_spring)
                        for i, t in enumerate(t_eval)])
        all_qp.append(qp)
        all_dqp.append(dqp)
        # Use last state as next initial (for continuity when samples > 1)
        x0 = qp[-1]

    x = np.concatenate(all_qp, axis=0)[:num_steps_total]
    dx = np.concatenate(all_dqp, axis=0)[:num_steps_total]
    ...
    return {
        'x': x[:n_train].astype(np.float32),
        'dx': dx[:n_train].astype(np.float32),
        'test_x': x[n_train:n_train + n_test].astype(np.float32),
        'test_dx': dx[n_train:n_train + n_test].astype(np.float32),
    }
