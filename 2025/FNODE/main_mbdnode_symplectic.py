#!/usr/bin/env python
"""
Main script for MBDNODE with symplectic integrators support.
Aligned with main_mbdnode.py structure for consistency.
"""
...
logger = logging.getLogger("MBDNODE_Symplectic")
...
# --- Imports ---
from Model.Data_generator import generate_dataset, generate_slider_crank_dataset, generate_training_data
from Model.utils import set_seed, get_output_paths, calculate_model_parameters, save_data, plot_trajectory_comparison, plot_sms_results
from Model.model import (MBDNODE, MBDNODE_Symplectic,
                        train_trajectory_MBDNODE, train_trajectory_MBDNODE_symplectic,
                        test_MBDNODE, test_MBDNODE_symplectic)

# --- Argument Parsing ---
def parse_arguments():
    parser = argparse.ArgumentParser(description="MBDNODE with Symplectic Integrators")
    ...
    parser.add_argument('--train_ratio', type=float, default=0.1, help="Training data ratio (default 0.1).")

    # Add generation_numerical_methods for data generation (like in symplectic version)
    parser.add_argument('--generation_numerical_methods', type=str, default='analytical',
                        choices=['analytical', 'fe', 'rk4', 'midpoint', 'sep_sv', 'yoshida4', 'fukushima6'],
                        help='Numerical method for data generation (use analytical when available)')

    # Model Hyperparameters
    parser.add_argument('--layers', type=int, default=3, help="Number of layers for NN models (default 3 for symplectic).")
    ...
    parser.add_argument('--activation', type=str, default='relu', choices=['relu', 'tanh'],
                        help="Activation function (default relu for symplectic).")
    ...
    # Numerical methods - now supports symplectic integrators
    parser.add_argument('--numerical_methods', type=str, default='rk4',
                        choices=['fe', 'rk4', 'midpoint', 'sep_sv', 'yoshida4', 'fukushima6'],
                        help="Numerical method for integration (including symplectic methods).")
    ...
    # Mark as symplectic if using symplectic methods
    symplectic_methods = ['sep_sv', 'yoshida4', 'fukushima6']
    args.is_symplectic = args.numerical_methods in symplectic_methods
...
def main():
    ...
    log_suffix = '_symplectic' if args.is_symplectic else ''
    log_file = os.path.join(log_dir, f'MBDNODE{log_suffix}.log')
    ...
    if args.is_symplectic:
        logger.info(f"Using Symplectic Integrator: {args.numerical_methods}")
    ...
    # Data generation or loading - aligned with main_mbdnode.py
    if args.generate_new_data or not os.path.exists(s_train_file):
        ...
        # Special handling for analytical methods or when CSV doesn't exist
        if args.generation_numerical_methods == 'analytical' and args.test_case == 'Single_Mass_Spring':
            # Generate using analytical method for SMS
            logger.info("Using analytical method for Single Mass Spring data generation")
            Ground_trajectory = generate_training_data(
                test_case=args.test_case,
                numerical_methods='analytical',
                dt=args.data_dt,
                num_steps=args.data_total_steps,
            )
            # Save to CSV format for consistency
            os.makedirs(dataset_path, exist_ok=True)

            # Reshape from [1, num_steps, state_dim] to [num_steps, state_dim]
            if Ground_trajectory.ndim == 3:
                Ground_trajectory_np = Ground_trajectory.permute(1, 0, 2).reshape(-1, expected_features).cpu().numpy()
            else:
                Ground_trajectory_np = Ground_trajectory.cpu().numpy()

            # Split into train and test
            train_data = Ground_trajectory_np[:num_steps_train]
            test_data = Ground_trajectory_np[num_steps_train:args.data_total_steps]

            # Save to CSV
            pd.DataFrame(train_data).to_csv(s_train_file, index=False)
            pd.DataFrame(test_data).to_csv(os.path.join(dataset_path, "s_test.csv"), index=False)

            # Create dummy time data
            t_train = np.arange(num_steps_train) * args.data_dt
            pd.DataFrame(t_train).to_csv(t_train_file, index=False)

        elif args.test_case == "Slider_Crank":
            ...
            generate_dataset(
                test_case=args.test_case,
                numerical_methods=args.generation_numerical_methods,
                dt=args.data_dt,
                num_steps=args.data_total_steps,
                seed=args.seed,
    ...
    # Always load from CSV files (matching main_mbdnode.py approach)
    ...
    # Initialize Model - use MBDNODE_Symplectic for symplectic methods
    if args.is_symplectic:
        model = MBDNODE_Symplectic(num_bodys=num_bodies, layers=args.layers, width=args.hidden_size,
                                  activation=args.activation, initializer=args.initializer).to(device)
        logger.info("Using MBDNODE_Symplectic model for symplectic integration")
    else:
        model = MBDNODE(num_bodys=num_bodies, layers=args.layers, width=args.hidden_size,
                      activation=args.activation, initializer=args.initializer).to(device)
        logger.info("Using standard MBDNODE model")
    ...
    if not args.skip_train:
        ...
        if args.is_symplectic:
            logger.info(f"=== MBDNODE Training with Symplectic Integrator: {args.numerical_methods} ===")
        else:
            logger.info(f"=== MBDNODE Training with Standard Integrator: {args.numerical_methods} ===")

        training_start_time = time.time()

        # Use appropriate training function based on model type
        if args.is_symplectic:
            # CRITICAL: Convert data from [q, v] to [q, p] format for symplectic training
            logger.info("Converting training data from [q, v] to [q, p] format for symplectic integration")
            s_train_qp = s_train.clone()

            # Get masses for the system (Single_Mass_Spring uses m=10.0)
            if num_bodies == 1:
                ...
                masses = torch.ones(num_bodies, dtype=torch.float32, device=device)

            # Convert velocity to momentum: p = m * v
            for i in range(s_train_qp.shape[0]):
                s_train_qp[i, :, 1] = s_train[i, :, 1] * masses

            trained_model = train_trajectory_MBDNODE_symplectic(
                test_case=args.test_case,
                numerical_methods=args.numerical_methods,
                model=model,
                body_tensor=s_train_qp,  # Use [q, p] format data
        ...
        training_time_data = {
            'model': f'MBDNODE_{args.numerical_methods}',
        ...
        # Save final model
        model_suffix = f"_symplectic_{args.numerical_methods}" if args.is_symplectic else f"_{args.numerical_methods}"
        final_model_path = os.path.join(output_paths["model"],
                                        f"MBDNODE_final{model_suffix}_{args.epochs}epochs.pth")
        torch.save({
            'epoch': args.epochs,
    ...
    # Use appropriate test function based on model type
    if args.is_symplectic:
        test_trajectory = test_MBDNODE_symplectic(
            numerical_methods=args.numerical_methods,
            model=model,
            body=initial_state,
    ...
    # Calculate MSE with extrapolation-focused evaluation
    ...
    if 'training_time_data' in locals():
        training_results_csv = os.path.join(output_paths["results"], f"training_results_{args.numerical_methods}.csv")
        import csv
        with open(training_results_csv, 'w', newline='') as f:
            ...
            writer.writerow([f'MBDNODE_{args.numerical_methods}', args.test_case, training_time_data['training_time'],
    ...
    # Use MBDNODE_<integrator> naming convention
    model_name = f"MBDNODE_{args.numerical_methods}"
    ...
    ground_truth_full = ground_trajectory.reshape(-1, num_bodies * states_per_body)
    save_data(
    ...
    logger.info(f"Saved {model_name} predictions and ground truth to results/{args.test_case}/")
    ...
    metrics_path = os.path.join(output_paths["results"], f"{model_name}_test_metrics.csv")
    ...
    plot_trajectory_comparison(
        ...
        model_predictions={model_name: test_trajectory},
        ...
        base_filename=f"{args.test_case}_{model_name}_comparison",
        num_epochs=args.epochs
    )

    phase_path = os.path.join(output_paths["figures"],
                              f"{args.test_case}_{model_name}_comparison_phasespace_epochs_{args.epochs}.png")
    time_path = os.path.join(output_paths["figures"],
                             f"{args.test_case}_{model_name}_comparison_timeseries_epochs_{args.epochs}.png")
    ...
    if args.test_case == 'Single_Mass_Spring':
        ...
        pred_traj_np = test_trajectory.reshape(-1, states_per_body).cpu().detach().numpy()
    ...
    logger.info(f"=== MBDNODE ({args.numerical_methods}) Run Complete ===")
