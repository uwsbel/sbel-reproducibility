...
% view 
xCoordinateMin = min(pos(:,1)) - 2*radius;
xCoordinateMax = max(pos(:,1)) + 2*radius;
yCoordinateMin = min(pos(:,2)) - 2*radius;
yCoordinateMax = max(pos(:,2)) + 2*radius;
zCoordinateMin = min(pos(:,3)) - 2*radius;
zCoordinateMax = max(pos(:,3)) + 2*radius;
view(67,7)
...
slidingFrDescprtion = sprintf('max Fr_s = %.2gN, Fr_k = %.2gN', slidingSlackStatic * slidingFrStiffness, slidingSlackKinetic * slidingFrDamping);
title(slidingFrDescprtion, 'FontSize', fontSize);


% plot plane
[plane_x, plane_y] = meshgrid(xCoordinateMin:0.1:xCoordinateMax, yCoordinateMin:0.1:yCoordinateMax);
plane_z = plane_x * 0 + plane_y * 0 + 0;
...
%     end
    ...
    xlim([xCoordinateMin, xCoordinateMax]);
    ylim([yCoordinateMin, yCoordinateMax]);
    zlim([zCoordinateMin, zCoordinateMax]);
