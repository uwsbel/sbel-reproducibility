...
etaRoll = 0.1;
Kr =4*etaRoll*mySphere.radius^2*Ke;
...
scenario = 'sphere_rolling_flat_surface';
Tend = 0.5; dt = 1e-4;
...
hdl.Position = [0.2802    0.0157    0.5479    0.8741];
...
%         mySphere.omic(3), M_spinning(3));
    ...
    if mySimulation.generateMovie == true &&  mod(i, floor(length(t)/mySimulation.movieLoops)) == 0
        ...
        hold on
        grid off
        myPlane.drawPlaneColor(-1,1,0.8);
