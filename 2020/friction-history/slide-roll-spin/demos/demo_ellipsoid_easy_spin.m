...
eta = 0;  % choose eta to be about 1

%eta_spin = 0.002;

scenario = sprintf('ellipsoid_easySpinning_eta_%dE-2_omic_10pi', eta_spin*100);
Tend = 0.75; dt = 1e-4;
...
w1Origin = [-0.5; -0.5; 0];
w1Normal = [1; 0; 0];
wall1 = planeClass(w1Normal, w1Origin);

w2Normal = [0;-1;0];
w2Origin = [0.5; 0.5; 0];
wall2 = planeClass(w2Normal, w2Origin);
...
% figHdl.Position = [0 0.4 0.77 0.81];
...
for i = 1:length(t)-1
    ...
    if myEllipsoid.isInContactWithPlane(myPlane) == true
        ...
        if initiationOfContact == true
            ...
            curv = myEllipsoid.evaluateCurv([myContact.CP_curr_local(1), myContact.CP_curr_local(2)], myEllipsoid.c, myEllipsoid.a);

            myFrictionModel = frictionModel(mu_s, mu_k, Ke, eta, eta_spin, forceNormal, myEllipsoid, mySimulation.dt, curv);
...
%     LW = 2;
    ...
    if mySimulation.generateMovie == true &&  mod(i, floor(length(t)/mySimulation.movieLoops)) == 0
        ...
        textHdl = text(min(xlim), min(ylim), max(zlim), ...
            sprintf('time = %.4gsec, KE=%g \n spinning mode:%s, Ts=%.2gN', ...
            t(i), myEllipsoid.getKineticEnergy, ...
            spinning_Tr.mode, spinning_Tr.totalFriction));
