...
//
// =============================================================================
// Author: Huzaifa Mustafa Unjhawala and Khailanii Slaton
// =============================================================================
// Normal Bevameter Validation Problem involving pressing a plate into granular
// material at a prescribed force and measuring the plate pressure vs sinkage.
// Comparing against GRC-1 paper:
// https://www.sciencedirect.com/science/article/pii/S0022489810000388
// =============================================================================
...
constexpr double nu_poisson = 0.3;
...
std::string ToFixedString(double value, int precision) {
    std::ostringstream oss;
...
std::string BuildOutputDirectory(const SimParams& params, double container_diameter) {
    std::string base_dir = GetChronoOutputPath() + "FSI_NormalBevameter_GRC1_" +
                           ToFixedString(params.container_height * 100.0, 1) + "cm_" + params.rheology_model_crm + "_" +
                           ToFixedString(container_diameter * 100.0, 1) + "cm";

    if (params.rheology_model_crm == "MCC") {
        base_dir += "_pre_pressure_scale_" + ToString(params.pre_pressure_scale);
    }

    std::vector<std::string> subdirs = {
        "maxPressure_" + ToFixedString(params.max_pressure / 1000.0, 2),
        "plateDiameter_" + ToFixedString(params.plate_diameter * 100.0, 2),
        "youngsModulus_" + ToString(params.y_modulus),
        "density_" + ToString(params.density),
        "mu_s_" + ToString(params.mu_s),
        "mu_2_" + ToString(params.mu_2),
        "cohesion_" + ToString(params.cohesion),
        "boundaryType_" + params.boundary_type,
        "viscosityType_" + params.viscosity_type,
        "kernelType_" + params.kernel_type,
    };

    filesystem::path out_dir(base_dir);
    for (const auto& subdir : subdirs) {
        out_dir /= subdir;
    }

    std::ostringstream run_tag;
    run_tag << "ps_" << params.ps_freq
            << "_s_" << params.initial_spacing
            << "_d0_" << params.d0_multiplier
            << "_t_" << params.time_step
            << "_av_" << params.artificial_viscosity;

    out_dir /= run_tag.str();
    return out_dir.string();
}

bool PrepareOutputDirectories(const std::string& out_dir, const SimParams& params) {
    if (!EnsureDirectory(out_dir)) {
        return false;
    }

    if (params.output) {
        if (!EnsureDirectory(filesystem::path(out_dir) / "particles"))
            return false;
        if (!EnsureDirectory(filesystem::path(out_dir) / "fsi"))
            return false;
    }

    if (params.output || params.snapshots) {
        if (!EnsureDirectory(filesystem::path(out_dir) / "vtk"))
            return false;
    }

    if (params.snapshots) {
        if (!EnsureDirectory(filesystem::path(out_dir) / "snapshots"))
            return false;
    }
...
void PrintProblemSpecs(const SimParams& params) {
...
void SimulateMaterial(int i, const SimParams& params) {
    ...
    // Register plate as FSI body with explicit BCE points
    ...
    // Add motor to push the plate at a force that ramps the pressure up to max pressure
    ...
    // Output directories
    std::string out_dir = BuildOutputDirectory(params, container_diameter);
    if (!PrepareOutputDirectories(out_dir, params)) {
        return;
    }
    ...
    if (params.render) {
        ...
        visVSG->AddCamera(ChVector3d(0, -1.4 * container_diameter, 0.7 * container_diameter), ChVector3d(0, 0, container_height));
    ...
    const std::string out_file = (filesystem::path(out_dir) / "force_vs_time.txt").string();
    ...
    while (time < t_end) {
        ...
        if (time >= pres_out_frame / pres_out_fps) {
            double plate_NetPressure = std::abs(plate->GetAppliedForce().z() / plate_area);
    ...
    plot_filename << out_dir << "/results_depth_h_" << std::fixed << std::setprecision(3) << container_height << "m.gpl";
