...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        # Keep feature shape stable for PointNav head regardless input resolution.
...
class PointNavResNetPolicy(nn.Module):
    def __init__(self, discrete_actions: bool = False) -> None:
        super().__init__()
    ...
    def act(
        ...
        if self.discrete_actions:
            if deterministic:
                action = torch.argmax(distribution.logits, dim=-1, keepdim=True)
            else:
