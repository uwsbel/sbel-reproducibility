...
class ValueMap(BaseMap):
    ...
    def sort_waypoints(
        ...
        def point_to_pixel(point: np.ndarray) -> Tuple[int, int]:
            x, y = point
            px = int(-x * self.pixels_per_meter) + self._episode_pixel_origin[0]
            py = int(-y * self.pixels_per_meter) + self._episode_pixel_origin[1]
            return (self._value_map.shape[0] - px, py)

        def pixel_in_bounds(point_px: Tuple[int, int]) -> bool:
            return (
                0 <= point_px[0] < self._value_map.shape[0]
                and 0 <= point_px[1] < self._value_map.shape[1]
            )

        def get_value(point_px: Tuple[int, int]) -> Union[float, Tuple[float, ...]]:
            all_values = [
        ...
        valid_waypoints: List[np.ndarray] = []
        values: List[Union[float, Tuple[float, ...]]] = []
        for point in waypoints:
            point_px = point_to_pixel(point)
            if not pixel_in_bounds(point_px):
                continue
            valid_waypoints.append(np.asarray(point))
            values.append(get_value(point_px))
        ...
        sorted_frontiers = np.array([valid_waypoints[i] for i in sorted_inds])
