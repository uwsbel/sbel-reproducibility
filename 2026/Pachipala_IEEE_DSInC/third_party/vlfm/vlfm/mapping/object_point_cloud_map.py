...
class ObjectPointCloudMap:
    ...
    def update_map(
        ...
        # Drop only the near points instead of discarding the entire detection.
        # This keeps large nearby objects (e.g., a bed at spawn) while still
        # suppressing self-hits and spurious near-field artifacts.
        min_valid_object_dist = 0.15
        point_dists = np.linalg.norm(global_cloud[:, :3] - curr_position, axis=1)
        global_cloud = global_cloud[point_dists >= min_valid_object_dist]
    ...
    def _extract_object_cloud(
        ...
        # If min_depth == 0, depth=0 is very likely invalid and can be treated as far.
        # If min_depth > 0, depth=0 may represent values clipped to min depth, so
        # keep them to avoid pushing close objects out to max depth.
        if min_depth <= 1e-6:
