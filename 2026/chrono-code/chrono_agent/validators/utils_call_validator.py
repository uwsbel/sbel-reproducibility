"""Static AST validator for calls to chrono_agent.utils.* functions.

Parses generated simulation code, resolves every name that refers to a
public `chrono_agent.utils` symbol (across the supported import styles),
...
from chrono_agent.utils._signature_digest import get_signatures, get_fq_signatures

logger = logging.getLogger(__name__)


_UTILS_PKG = "chrono_agent.utils"


@dataclass
class UtilsCallIssue:
    """A single mismatched call to a chrono_agent.utils symbol."""
...
def format_issues_for_feedback(issues: List[UtilsCallIssue]) -> str:
    ...
    if not issues:
        return ""
    lines = [
        "Static validation found incorrect calls to chrono_agent.utils API. "
...
class _ImportResolver(ast.NodeVisitor):
    ...
    Supports:
      * `from chrono_agent.utils import X`                        -> X
      * `from chrono_agent.utils import X as Y`                   -> Y -> X
      * `from chrono_agent.utils.scene_assets import X`           -> X (fq via submodule)
      * `from chrono_agent.utils.scene_assets import X as Y`      -> Y -> X
      * `import chrono_agent.utils as u`                          -> u.X
      * `import chrono_agent.utils.scene_assets as sa`            -> sa.X
      * `import chrono_agent.utils`                               -> chrono_agent.utils.X
      * `import chrono_agent.utils.scene_assets`                  -> chrono_agent.utils.scene_assets.X
    """
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:  # noqa: N802 (ast API)
        ...
        for alias in node.names:
            ...
            # For `from chrono_agent.utils import X` we trust the short name;
            # for `from chrono_agent.utils.submod import X` we also key on the
    ...
    def resolve_call(self, func: ast.AST) -> Optional[Tuple[str, str]]:
        ...
        if isinstance(func, ast.Attribute):
            ...
            # Case 1: `u.X` where u is alias for chrono_agent.utils(.sub)
            ...
            if mod_path is not None:
                ...
                if not _looks_like_utils(full_module):
                    return None
                # Package-level access (`u.X` where u == chrono_agent.utils) goes
            ...
            # `chrono_agent.utils.scene_assets.X` when user wrote
            # `import chrono_agent.utils.scene_assets`. The head `chrono_agent`
            # is recorded verbatim in _module_aliases (see visit_Import).
            if head == "chrono_agent":
