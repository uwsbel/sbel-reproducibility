"""
Main CLI entry point for Chrono-Agent.
"""
...
from chrono_agent.config import get_settings, init_directories_sync
from chrono_agent.agents.exceptions import (
    PlanModificationIncompleteError,
    PlanModificationValidationError,
)
from chrono_agent.models.plan import SimulationPlan
from chrono_agent.utils.logger import setup_logger
from chrono_agent.utils.terminal_ui import AgentOutputManager, create_output_manager
from chrono_agent.utils.textual_tui import is_textual_available, run_tui
...
def check_environment():
    ...
    if result.returncode != 0:
        ...
        console.print("[yellow]Please activate the chrono-agent environment:[/yellow]")
        console.print("  conda activate chrono-agent")
        if result.stderr:
    ...
    # Optional: warn if conda env name is not chrono-agent
    conda_env = os.environ.get("CONDA_DEFAULT_ENV", "unknown")
    if conda_env != "chrono-agent":
        console.print(
            f"[yellow]WARNING: Running in '{conda_env}' environment, not 'chrono-agent'[/yellow]"
        )
...
def _show_cli_header() -> None:
    ...
    console.print(Panel.fit(
        "[bold cyan]Chrono-Agent[/bold cyan]\n"
...
def codegen_from_plan(
    ...
    try:
        from chrono_agent.agents import CodeGenerationAgent
...
def run_from_plan(
    ...
    try:
        from chrono_agent.workflow.engine import run_workflow
        ...
        def run_with_spinner(description: str, coroutine):
            ...
            from chrono_agent.ui import LiveEventRenderer
...
async def _cli_plan_approval(state: dict) -> dict:
    ...
    def _count_inline_tokens(p: dict) -> int:
        ...
        """
        from chrono_agent.models.plan_markdown_parser import TOKEN_RE
    ...
    while True:
        ...
        if normalized.startswith("/"):
            from chrono_agent.utils.slash_commands import (
                ACTION_EXIT,
                SlashContext,
        ...
        if _plan_agent is None:
            from chrono_agent.agents import PlanningAgent
...
def generate(
    ...
    Example:
        chrono-agent "Create a bouncing ball simulation"
    """
...
def _render_pipeline_stats_panel(result: dict) -> None:
    ...
    from chrono_agent.ui.renderer import (
        _AGENT_COLORS,
...
def version():
    ...
    console.print("[bold]Chrono-Agent[/bold] version 0.1.0")
