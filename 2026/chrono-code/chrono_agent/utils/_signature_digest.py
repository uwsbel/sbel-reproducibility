"""Runtime signature digest for chrono_agent.utils public API.

Scans the symbols in `chrono_agent.utils.__all__` once per process and exposes:
...
_UTILS_PACKAGE = "chrono_agent.utils"
...
def get_fq_signatures() -> Dict[str, inspect.Signature]:
    ...
    Used when the agent imports via `import chrono_agent.utils.scene_assets`
    and then calls `chrono_agent.utils.scene_assets.add_visual_assets(...)`.
    """
    out: Dict[str, inspect.Signature] = {}
...
def build_reference_block() -> str:
    ...
    # Group by submodule (everything after `chrono_agent.utils.`) so related
    ...
    lines: List[str] = []
    lines.append("UTILS API REFERENCE (chrono_agent.utils — always prefer these over reinventing):")
    lines.append(
        "Import: `from chrono_agent.utils import <name>` or "
        "`from chrono_agent.utils.<submodule> import <name>`."
    )
    lines.append("")
