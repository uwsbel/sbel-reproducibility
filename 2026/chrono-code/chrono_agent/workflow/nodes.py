...
from chrono_agent.agents import PlanningAgent, CodeGenerationAgent, ExecutionAgent, ReviewAgent
from chrono_agent.agents.exceptions import AgentLLMError
from chrono_agent.workflow.state import WorkflowState, make_agent_message
from chrono_agent.workflow.conditions import DEFAULT_MAX_EXECUTION_RETRIES
from chrono_agent.workflow.events import emit_custom_event, emit_progress_event
from chrono_agent.models.handoff import LLMHandoff, StructuredError
from chrono_agent.models.plan import SimulationPlan, SimulationStep
from chrono_agent.models.plan_markdown_parser import has_unresolved_tokens
from chrono_agent.models.code import CodeArtifact, GeneratedCode
from chrono_agent.models.execution import ExecutionResult
from chrono_agent.config import get_settings
from chrono_agent.utils.error_utils import fingerprint_error
from chrono_agent.utils.logger import log_boxed
...
async def execution_node(state: WorkflowState) -> WorkflowState:
    ...
    if settings.enable_headless_mode:
        try:
            from chrono_agent.utils.display_server import DisplayServer
            from chrono_agent.utils.frame_capture import XvfbFrameCapture
...
async def physics_analysis_node(state: WorkflowState) -> WorkflowState:
    ...
    if plan_obj.plan_type == "scene" and placement_csv.exists():
        try:
            from chrono_agent.validators.scene_placement import validate_scene_placement
