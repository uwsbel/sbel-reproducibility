---
name: camera
description: Attach an RGB preview+recording camera to a body using the project helper setup_preview_camera (returns a CameraRecorder). NON-FSI scenes only — for FSI/SPH scenes use fsi/sph Pattern G + chrono_agent.utils.vsg_recording instead, since sensor cameras (OptiX) cannot render SPH particles.
...
`chrono_agent.utils.vsg_recording` (`hide_vsg_gui`, `lock_side_camera`,
...
(see `chrono_agent/models/plan.py` — `SimulationStep.cameras:
