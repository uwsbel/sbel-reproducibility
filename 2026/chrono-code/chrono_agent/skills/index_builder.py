...
* Utils: AST-scan ``chrono_agent/utils/*.py`` for public ``def``s.
...
serialize to ``chrono_agent/skills/.skill_index.json`` so rebuilds are
cheap (~tens of ms per run; rebuilt on every process start for
simplicity — this is not a hot path).

Used by :mod:`chrono_agent.tools.signature_injector` (Step 8) at
...
class UtilEntry:
    ...
    module: str     # e.g. "chrono_agent.utils.scene_assets"
...
def build_utils_signature_index(utils_root: Optional[Path] = None) -> Dict[str, UtilEntry]:
    """Scan ``chrono_agent/utils/*.py`` for public ``def``s and return
    ...
    for py_file in sorted(root.glob("*.py")):
        if py_file.name.startswith("_"):
            continue
        module_name = f"chrono_agent.utils.{py_file.stem}"
        try:
