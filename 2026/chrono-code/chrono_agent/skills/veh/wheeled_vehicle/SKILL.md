...
```python
from chrono_agent.utils.vehicle_geometry import chassis_init_z
...
```python
from chrono_agent.utils.vehicle_geometry import assert_vehicle_on_support
...
| Body kind | Pattern |
|---|---|
| Container/tank walls | Use `chrono_agent.utils.fsi_assets.build_fsi_tank`; do not hand-roll wall BCE. |
...
Prefer the helper from `chrono_agent.utils.fsi_assets`; it wires the vehicle,
shared MBS system, and SPH plugin into one vehicle-aware VSG visualizer.

```python
from chrono_agent.utils.fsi_assets import build_fsi_vehicle_visualizer
...
`chrono_agent.utils.add_collision_via_subbodies` with `CollisionType_NONE`.
