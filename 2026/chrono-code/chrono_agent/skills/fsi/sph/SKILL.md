...
canonical_examples:
  ...
  - "Container: build = chrono_agent.utils.fsi_assets.build_fsi_tank(sysMBS, sysSPH, sysFSI, world_extent=ChAABB(...), contact_material=mat, bce_layers=ChVector3i(2,0,-1))"
  - "Fluid seeding: points = chrono.ChGridSamplerd(d0).SampleBox(build.sampler_box_center, build.sampler_box_halfdim)"
  - "Floating body: build = chrono_agent.utils.fsi_assets.build_fsi_floating_box(sysMBS, sysSPH, sysFSI, world_center=ChVector3d(...), size=ChVector3d(...), density=400, contact_material=mat)"
  - "Pre-Initialize guard: chrono_agent.utils.fsi_assets.assert_fsi_bodies_unique(sysMBS) — call right before sysFSI.Initialize() to catch double-AddBody on build_fsi_* bodies (silent BCE desync → fluid leaks through walls)"
...
**Always use `chrono_agent.utils.fsi_assets.build_fsi_tank`.** It takes
...
```python
from chrono_agent.utils.fsi_assets import build_fsi_tank
...
```python
from chrono_agent.utils.fsi_assets import assert_fsi_bodies_unique
...
`chrono_agent.utils.fsi_assets.build_fsi_floating_box`:

```python
from chrono_agent.utils.fsi_assets import build_fsi_floating_box
...
from chrono_agent.utils import run_recording_loop
from chrono_agent.utils.fsi_assets import build_fsi_tank, build_fsi_floating_box, assert_fsi_bodies_unique
from chrono_agent.utils.vsg_recording import (
    setup_vsg_recording, lock_side_camera, hide_vsg_gui,
)
...
```python
from chrono_agent.utils.fsi_assets import SphMotionLogger
...
``chrono_agent.utils.scene_assets`` for ad-hoc diagnostic use, but
...
- For utility helpers (`chrono_agent.utils.*`), trust the function's
...
- For sanity-critical Z / TOP / BOTTOM / CENTER values, derive them
  ...
  `chrono_agent.utils.vehicle_geometry.support_top_z_from_body(body)`
