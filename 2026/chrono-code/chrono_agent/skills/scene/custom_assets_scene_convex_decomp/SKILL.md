...
`chrono_agent.utils.scene_assets` utilities — `AssetDescriptor`,
...
- chrono_agent.utils.scene_assets.create_asset_body             # Scenario 1: repo-local data/scene/ assets
- chrono_agent.utils.scene_assets.add_visual_assets             # Scenario 2: sensor/offroad/ outdoor assets
- chrono_agent.utils.scene_assets.add_collision_from_decomposition
- chrono_agent.utils.scene_assets.add_collision_via_subbodies   # vehicle chassis sub-body collision (see veh/wheeled_vehicle)
- chrono_agent.utils.scene_assets.convex_decompose_asset
- chrono_agent.utils.scene_assets.ensure_convex_json            # auto-runs inside add_visual_assets when collision=True; rarely called directly
- chrono_agent.utils.scene_assets.load_convex_hulls
- chrono_agent.utils.scene_assets.transform_vertices
- chrono_agent.utils.scene_assets.make_contact_material
- chrono_agent.utils.scene_assets.box_inertia
- chrono_agent.utils.scene_assets.write_placement_csv
- chrono_agent.utils.scene_assets.write_contacts_csv
- chrono_agent.utils.scene_assets.write_links_csv
- chrono_agent.utils.scene_assets.AssetDescriptor
- chrono_agent.utils.scene_placement.FootprintRegistry
- chrono_agent.utils.scene_placement.SurfaceStack
- chrono_agent.utils.setup_preview_camera
- chrono_agent.utils.run_recording_loop
...
from chrono_agent.utils.scene_assets import (
    create_asset_body,
    add_visual_assets,
    AssetDescriptor,
...
from chrono_agent.utils.scene_placement import FootprintRegistry, SurfaceStack
from chrono_agent.utils import setup_preview_camera   # MUST be called — see §7
```
...
**Banned inline definitions**: Do NOT define `load_convex_hulls`, `transform_vertices`, `add_collision_from_json`, `add_collision_from_decomposition`, `add_collision_debug_visuals`, `quat_from_angle_x`, `quat_from_angle_z`, `quat_mul`, `box_inertia`, `write_placement_csv`, `write_contacts_csv`, `write_links_csv`, or `ContactReporter` in the generated `simulation.py`. All of these are provided by `chrono_agent.utils.scene_assets`. Camera setup is provided by `chrono_agent.utils.setup_preview_camera` — do NOT construct `ChCameraSensor` or filters manually.
...
All scene assets MUST be created through `create_asset_body()` from `chrono_agent.utils.scene_assets`. **Do NOT scatter body creation logic across multiple ad-hoc blocks or reimplement the mesh loading/transform pipeline inline.**
...
1. **Imports are fixed**: `from chrono_agent.utils.scene_placement import FootprintRegistry, SurfaceStack`.
...
```python
from chrono_agent.utils.scene_placement import FootprintRegistry
...
`chrono_agent.utils.scene_assets` utilities as the indoor workflow
...
```python
from chrono_agent.utils import make_contact_material
...
These are imported from `chrono_agent.utils.scene_assets`. Do NOT redefine `ContactReporter`, `write_placement_csv`, `write_contacts_csv`, or `write_links_csv` inline.
...
- Do not redefine utility functions inline — import from `chrono_agent.utils.scene_assets`.
