...
| `scene/custom_assets_scene_convex_decomp` | **Required when the scene includes outdoor props (trees / rocks / cottage / foliage) OR indoor office assets.** Authoritative reference for `chrono_agent.utils.scene_assets` — `AssetDescriptor` + `add_visual_assets` placement, per-asset-family collision rules, support-plane recipe for dynamic props on SCM, `FootprintRegistry` for deterministic non-overlap |
...
- All assets must be implemented through project utilities from `chrono_agent.utils` / `chrono_agent.utils.scene_assets`; the owning child skill decides which helper is valid.
