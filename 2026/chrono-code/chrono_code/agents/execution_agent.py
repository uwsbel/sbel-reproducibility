...
from chrono_code.utils.async_fs import ensure_dir
...
class ExecutionAgent(BaseAgent):
    ...
    async def execute(
        ...
        try:
            from chrono_code.utils.vsg_title import rewrite_window_titles
            ...
            # the interpreter is inside the chrono-code env.
            ...
            try:
                async def _stream_pipe(pipe, pipe_name: str) -> str:
                    ...
                    from chrono_code.workflow.events import emit_subprocess_event
    ...
    def _structure_runtime_error(
        ...
        if m_attr:
            ...
            failing_symbol = f"{fn}()"
            introspection_hint = (
                f"bash('grep -n \"def {fn}\" chrono_code/utils/*.py') "
    ...
    def _recover_vsg_frames(self) -> None:
        ...
        """
        try:
            from chrono_code.utils.vsg_recording import encode_orphan_vsg_frames
