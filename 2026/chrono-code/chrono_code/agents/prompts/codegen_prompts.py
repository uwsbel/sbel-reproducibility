...
VIDEO_GENERATION_RULE_VSG_ONLY = (
    ...
    "`from chrono_code.utils import run_recording_loop` plus "
    "`from chrono_code.utils.vsg_recording import setup_vsg_recording, "
...
12) UTILS FUNCTIONS (REQUIRED): Use project utilities from chrono_code.utils; do NOT reimplement them inline. Always: `from chrono_code.utils import run_recording_loop` and `from chrono_code.utils.scene_assets import add_visual_assets, AssetDescriptor, create_asset_body, make_contact_material`. The recording-related imports DEPEND on the recording_mode picked in rule 8 above — see that rule for the mode-specific helpers (`setup_preview_camera` for sensor_cams; `setup_vsg_recording` / `lock_side_camera` / `hide_vsg_gui` for vsg_only). Main loop MUST be driven by `run_recording_loop(sys, duration=..., time_step=..., vis=..., manager=..., render_fps=50.0, step_fn=..., recorders=...)` — never hand-write `while vis.Run():`. `make_contact_material` takes friction/restitution/method and NEVER a system argument.
