...
//
// =============================================================================
// Author: Wei Hu, Radu Serban, Huzaifa Mustafa Unjhawala, and Khailanii Slaton
// =============================================================================
// Sloped single wheel RTF benchmark. A Viper wheel is driven through CRM terrain
// (optionally on a slope, applied by tilting gravity) while drawbar pull, torque,
// slip, and real-time-factor are recorded.
// =============================================================================
...
std::string BuildOutputDirectory(const SimParams& params) {
    std::string benchmark_root = GetChronoOutputPath() + "FSI_SlopedSingleWheelRTFBenchmark";
...
bool PrepareOutputDirectories(const std::string& out_dir, const SimParams& params) {
    ...
    if (params.output && params.write_marker_files) {
...
//------------------------------------------------------------------
// Generate BCE markers on the wheel body and its grousers
//------------------------------------------------------------------
...
void CreateSolidPhase(ChFsiSystemSPH& sysFSI,
    ...
    // Generate BCE markers on the wheel and grousers
...
int main(int argc, char* argv[]) {
    ...
    if (params.benchmark_tag.empty()) {
        ...
        tag << std::fixed << "depth_" << std::setprecision(3) << params.container_depth << "_slope_"
            << params.slope_angle << "_rheology_" << params.rheology_model_crm << "_sim_" << params.sim_number;
    ...
    const std::string out_dir = BuildOutputDirectory(params);
    ...
    CreateSolidPhase(sysFSI, wheel_IniPos, wheel_IniVel, params.wheel_AngVel, params.render, params.grouser_height,
                     params.total_mass, params.initial_spacing);
