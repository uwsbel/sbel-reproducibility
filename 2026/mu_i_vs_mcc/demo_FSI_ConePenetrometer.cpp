...
// and measuring the force on the cone tip
...
struct ConeProperties {
    ...
    double diameter = std::sqrt(surface_area * 4 / CH_PI);
...
std::string BuildOutputDirectory(const SimParams& params) {
    std::string base_dir = GetChronoOutputPath() + "FSI_ConePenetrometer_GRC1_" +
                           ToFixedString(params.container_height * 100.0, 1) + "cm_" +
                           params.rheology_model_crm + "_" + ToString(params.pre_pressure_scale);

    std::vector<std::string> subdirs = {
        "penetrationDepth_" + ToFixedString(params.penetration_depth, 2),
...
double ComputeConePressure(std::shared_ptr<ChBody> cone, const ConeProperties& coneProp) {
    const double cone_area = CH_PI * std::pow(coneProp.diameter / 2.0, 2);
    return std::abs(cone->GetAppliedForce().z() / cone_area);
}
...
void SimulateMaterial(int i, const SimParams& params, const ConeProperties& coneProp) {
    ...
    // Output directories
    std::string out_dir = BuildOutputDirectory(params);
    ...
    while (time < t_end) {
        ...
        if (params.output && time >= out_frame / params.output_fps) {
            ...
            std::cout << "Current Depth: " << current_depth << std::endl;
            double cone_pressure = ComputeConePressure(cone, coneProp);
